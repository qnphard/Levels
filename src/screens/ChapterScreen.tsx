...
function ChapterScreenContent({ chapterId }: { chapterId: string }) {
  ...
  const relatedChapters = chapter?.relatedChapters
  ...
  // Show error only if there's no content at all
  if (!content) {
    return (
  ...
  // Use chapter metadata if available, or fallback for articles
  const glowColor = chapter?.glowColor
    ? theme.feelingsChapters[chapter.glowColor]
    : theme.primary;
